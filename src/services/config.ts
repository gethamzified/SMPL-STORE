...
const fetchConfig = async (): Promise<StoreConfig> => {
    ...
    try {
        ...
        if (configError) {
            console.error('[StoreConfigService] DB Error:', configError);
            throw new AppError(configError.message, 'DB_ERROR');
        }
        ...
        console.error('[StoreConfigService] Fetch Failed:', err);
        throw new AppError(err.message || 'Store configuration fetch failed', 'DB_ERROR');
    }
};

...
import { OrderReviewButton } from "@/components/orders/OrderReviewButton";
import { outlinedBorder, outlinedPanel } from "@/lib/outline";
...
export default async function OrderDetailPage({ params }: Props) {
    ...
    return (
        <div className="max-w-4xl mx-auto px-4 pb-24">
            {/* Minimal Header */}
            <header className="py-8 border-b border-[#1a1a1a] mb-12">
                <Link
                    href="/account/orders"
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    ...
                    <div className="flex items-center gap-4">
                        <div className={`inline-flex items-center gap-2 px-4 py-2 border border-[#1a1a1a] ${statusConfig.bg} ${statusConfig.text} text-[10px] font-bold uppercase tracking-[0.1em]`}>
            ...
            {order.tracking_number && (
                <div className="bg-black text-white p-6 mb-12 flex flex-col sm:flex-row items-center justify-between gap-4 border border-[#1a1a1a]">
                    <div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-12">
                ...
                <div className="lg:col-span-3 space-y-8">
                    <h2 className="text-[10px] font-bold uppercase tracking-[0.2em] text-neutral-400 pb-4 border-b border-[#1a1a1a]">
                        Items Ordered
                    </h2>

                    <div className={`space-y-0 divide-y divide-[#1a1a1a] p-6 ${outlinedPanel}`}>
                        {order.items.map((item: any) => (
                            <div key={item.id} className="flex gap-6 py-6 first:pt-0">
                                <div className="relative w-20 h-24 bg-neutral-100 flex-shrink-0 overflow-hidden border border-[#1a1a1a]">
                                    {item.image_url ? (
                                        <Image
                                            src={item.image_url}
                                            alt={item.title}
                                ...
                                <div className="flex-1 flex flex-col justify-between min-w-0">
                                    <div className="flex justify-between items-start gap-4">
                                        ...
                                        {order.status === 'delivered' && (
                                            <OrderReviewButton productId={item.product_id} productTitle={item.title} />
                                        )}
                                    </div>
                ...
                <div className="lg:col-span-2 space-y-8">
                    {/* Summary */}
                    <div className={`p-6 space-y-4 ${outlinedPanel}`}>
                        <h3 className="text-[10px] font-bold uppercase tracking-[0.2em] text-neutral-400 pb-3 border-b border-[#1a1a1a]">
                            Summary
                        </h3>
                    ...
                    <div className="space-y-3">
                        ...
                        <div className={`p-4 text-sm ${outlinedBorder}`}>
                    ...
                    <div className="space-y-3">
                        ...
                        <div className={`p-4 text-sm space-y-1 ${outlinedBorder}`}>
                    ...
                    {/* Contact */}
                    <div className="text-sm text-neutral-500 pt-4 border-t border-[#1a1a1a] space-y-1">
                        <p>{order.email}</p>
